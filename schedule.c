...
void getDepartureTime(Train* ptrTrain);
void resetSeat();
void deleteTrain();
		...
		for (int j = 0; j < MAX_STATION; j++) {
			for (int k = 0; k < MAX_COACH; k++) {
				for (int l = 0; l < MAX_SEAT; l++) {
					strcpy(train.station[j].coach[k].seat[l].status, "Available");
				}
			}
		}
...
void printSchedule() {
	...
	while (fread(&train, sizeof(Train), 1, schedule) != 0) {
		if (train.trainNumber % 2 == (option - 1) && train.trainNumber != 0) {
					...
					for (j = 0; j < ticket.pax; j++) {
						...
						if (departureStation < arrivalStation) {
							while (departureStation < arrivalStation) {
								strcpy(train.station[departureStation].coach[checkCoach(j, ticket)].seat[ticket.tDetail.seatDetail[j].seatNumber - 1].status, "Booked");
